...
export const signout = async(req,res,next)=>{
  try {
    res.clearCookie("access_token").status(200).json("User has been Log Out successfully")
  } catch (error) {
    next(error)
  }
}
...
export const getUsers= async(req,res,next)=>{
  ...
  try {
     const startIndex = parseInt(req.query.statrIndex) ||0
  ...
  const lastMonthUser = await User.countDco
